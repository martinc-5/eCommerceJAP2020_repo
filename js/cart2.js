...
function mostrarCarrito(array) {
    ...
    if (localStorage["guardarCarrito"] != null) {
      document.getElementById("itemsCarrito").innerHTML = JSON.parse(
        localStorage["guardarCarrito"]
      );
  
      for (let i = 0; i < array.length; i++) {
        document.getElementById(`cantidad${i}`).value = JSON.parse(
          localStorage[`guardarCantidad${i}`]
        );
      }
    }
    ...
    //  Si no hay un carrito guardado en localStorage crea uno nuevo y lo guarda en localStorage
      ...
      for (let i = 0; i < array.length; i++) {
        localStorage[`guardarCantidad${i}`] = document.getElementById(
          `cantidad${i}`
        ).value;
      }
    }
  ...
  document
    ...
    .addEventListener("click", function (e) {
      ...
      if (envioGuardado != "" && pagoGuardado != "") {
        swal("Felicidades!", "Que disfrutes tu compra!", "success");
      }
